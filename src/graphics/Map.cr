module Crystal2Day
  ...
  class Map < Crystal2Day::Drawable
    ...
    getter vertex_indices = [] of Int32
    ...
    def generate_vertices(view_width : UInt32, view_height : UInt32)
      @vertices = Array(LibSDL::Vertex).new(size: view_width * view_height * 4) {LibSDL::Vertex.new}
      @vertex_indices = Array(Int32).new(size: view_width * view_height * 6) {|i| (i // 6) * 4 + [0, 1, 2, 0, 2, 3][i % 6]}
    end
    ...
    def draw_directly
      LibSDL.render_geometry(@texture.renderer_data, @texture.data, @vertices, @vertices.size, @vertex_indices, @vertex_indices.size)
    end
  end
end
